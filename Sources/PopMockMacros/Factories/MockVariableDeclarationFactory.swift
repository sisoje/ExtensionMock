...
struct MockVariableDeclarationFactory {
    ...
    @MemberBlockItemListBuilder
    func mockVariableBodies(_ variables: [VariableDeclSyntax]) -> MemberBlockItemListSyntax {
        for variable in variables {
            mockVariableBody(variable)
        }
    }

    @MemberBlockItemListBuilder
    func mockVariableBody(_ variable: VariableDeclSyntax) -> MemberBlockItemListSyntax {
